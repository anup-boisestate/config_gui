...
class GIN3DConfigWin:
    ...
    def apply_cfg(self):
        ...
        # Mesh Size
        try:
            dimensions = self.cfgVal('Dimensions')
        ...
        # Turbulence
        try:
            turbulence = self.cfgVal('Turbulence')
            if turbulence is not None:
                turbulence = eval(turbulence)
            else:
                turbulence = False
        except Exception as err:
            log(err, 'e')
        ...
        # Temperature
        try:
            temperature = self.cfgVal('Temperature')
